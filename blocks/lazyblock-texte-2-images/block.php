    ...
    <div class="container flex flex-col gap-64">
        <div class="content w-full lg:w-2/3 z-10">
            <?php if (!empty($attributes['pretitle'])) { ?>
                <span class="pretitle-1 text-copper-red-600"><?= esc_html( $attributes['pretitle'] ) ?></span>
            <?php } ?>
            ...
            <?php if ($attributes["button"]) { ?>
                <a href="<?= $attributes['button-link'] ?>" class="btn btn-tertiary mt-16"><?= esc_html( $attributes['button-label'] ) ?></a>
            <?php } ?>
        </div>
